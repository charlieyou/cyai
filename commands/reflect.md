...
You are going to analyze multiple diary entries to identify recurring patterns, synthesize insights, and propose updates to the user's AGENTS.md file.

## Steps to Follow

1. **Check processed entries log**:

   - Read `.agents/config/reflections/processed.log` to find already-processed diary entries
   - Format: `[diary-filename] | [reflection-date] | [reflection-filename]`
   - Example: `2025-11-07-abc123.md | 2025-11-08 | 2025-11-reflection-1.md`
   - If file doesn't exist, all entries are unprocessed
   - Create the file if it doesn't exist: `touch .agents/config/reflections/processed.log`

2. **Locate diary entries**:

   - Directory: `.agents/config/diary/`
   - Entries are named: `YYYY-MM-DD-[thread-id].md`
...
5. **Create the reflections directory** (if it doesn't exist):
   ...
   - Directory: `.agents/config/reflections/`
   - Use `mkdir -p` to create it automatically

6. **Read current AGENTS.md to check for existing rules**:

   - Read `AGENTS.md` to understand what rules already exist
...
7. **Analyze entries for patterns AND rule violations**:
   ...
   - **CRITICAL - Rule Violation Detection**: Check if diary entries show violations of EXISTING AGENTS.md rules
     ...
     - If a diary mentions user correcting the agent for violating an existing rule, this is HIGH PRIORITY
     - Example: AGENTS.md says "no AI attribution" but diary shows "User corrected: Don't add AI attribution"
...
- PR descriptions: no AI tool attribution or mentions
...
Below are proposed additions to your `AGENTS.md` file. **Review these carefully before adding them.**

### Section: [General Preferences / Project-Specific / Code Quality / Git Workflow / etc.]

```markdown
[Proposed succinct bullet points for AGENTS.md]
...
9. **Save the reflection document**:
   ...
   - Save to: `.amp/config/reflections/[filename]`
...
10. **Automatically update AGENTS.md**:
   ...
   **PRIORITY 2: Add new rules**
   - THEN, append the new proposed rules to `AGENTS.md`
...
11. **Update processed entries log**:
   - Append processed diary entries to `.amp/config/reflections/processed.log`
...
12. **Present completion summary to user**:
   ...
   - List the AGENTS.md sections that were updated
...
4. **Actionability matters**: Only propose rules that the agent can actually follow
...
- ✅ Is this actionable? (the agent can actually do it)
...
**Default behavior**: Skip entries already listed in `.amp/config/reflections/processed.log`
